...
get_version:
	...
	/*
	 * Default: only register when ACPI selected apple_gmux. On MacBookPro11,3
	 * acpi_backlight=vendor (common leftover cmdline) makes apple_bl take
	 * this path instead, and apple_bl cannot drive the Retina PWM — so
	 * nothing exports /sys/class/backlight. Always register gmux backlight
	 * on this dual-GPU Retina; gmux is the only working PWM owner.
	 */
	register_bdev = true;
#endif
	if (register_bdev) {
